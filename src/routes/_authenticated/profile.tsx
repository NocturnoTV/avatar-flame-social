...
import { Button, Input, Label, Select, Sheet, Textarea } from "@/components/ui-kit";
      ...
      <Sheet open={editOpen} onClose={() => setEditOpen(false)} title={t("editProfile")}>
        <div className="max-h-[75vh] space-y-5 overflow-y-auto pb-2">
          ...
          <div className="space-y-4 rounded-2xl border border-border bg-card p-4">
            <p className="font-black">{t("profileEditIdentityTitle")}</p>
            <div>
              <Label>{t("username")}</Label>
              <Input value={p?.username ? `@${p.username}` : ""} disabled readOnly />
            </div>
            <div>
              <div className="flex items-center justify-between">
                <Label className="mb-0">{t("profileAgeLabel")}</Label>
                <span className="text-sm font-bold text-muted-foreground">{p?.age ?? "—"}</span>
              </div>
              <button
                type="button"
                onClick={() => patch({ age_visible: !(p?.age_visible ?? true) })}
                className={cn(
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    p?.age_visible ?? true ? "bg-primary" : "bg-surface-2",
                  )}
                >
                  <span
                    className={cn(
                      "absolute top-0.5 h-4 w-4 rounded-full bg-white transition",
                      p?.age_visible ?? true ? "left-4" : "left-0.5",
                    )}
                  />
                </span>
              </button>
            ...
            <div>
              <Label>
                {t("profileSpokenLanguages")} ({(p?.spoken_languages ?? []).length}/3)
              </Label>
              <p className="mb-2 text-xs text-muted-foreground">{t("profileSpokenLanguagesHint")}</p>
              <div className="flex flex-wrap gap-2">
                {LANGUAGES.map((l) => {
                  const current = p?.spoken_languages ?? [];
                  const active = current.includes(l.code);
                  return (
                    <button
                      key={l.code}
                      type="button"
                      onClick={() => {
                        if (active) {
                          patch({ spoken_languages: current.filter((c) => c !== l.code) });
                          return;
                        }
                        if (current.length >= 3) {
                        ...
                        patch({ spoken_languages: [...current, l.code] });
                      }}
                      className={cn(
          ...
          <div className="space-y-2 rounded-2xl border border-border bg-card p-4">
            <p className="font-black">{t("profileLookingForTitle")}</p>
            <Select
              value={p?.spark_looking_for ?? ""}
              onChange={(e) => patch({ spark_looking_for: e.target.value || null })}
            >
          ...
          <div className="space-y-2 rounded-2xl border border-border bg-card p-4">
            <p className="font-black">{t("profileCommunicationTitle")}</p>
          ...
          <div className="space-y-2 rounded-2xl border border-border bg-card p-4">
            <p className="font-black">{t("profileAvailabilityTitle")}</p>
            <div className="flex flex-wrap gap-2">
              {(
                [
                ...
                const current = p?.spark_availability ?? [];
